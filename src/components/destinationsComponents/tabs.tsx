...
const Tabs: React.FC<TabsProps> = ({ tabs, activeTab, setActiveTab, sectionRefs }) => {
            ...
            <div className="w-full px-4 py-3 sm:py-4 flex justify-center">
                <ul className="flex flex-wrap justify-center space-x-4 sm:space-x-6 text-gray-800 text-sm sm:text-base font-medium">
                    {tabs.map((tab) => (
                        <li
                            key={tab.name}
                            className={`cursor-pointer transition ${activeTab === tab.name ? "text-blue-600 font-bold" : "hover:text-blue-600"
                                }`}
